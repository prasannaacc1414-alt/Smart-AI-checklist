...
// On Vercel, rewrites can change req.url (e.g. /api/tasks -> /api).
...
app.use((req, _res, next) => {
  ...
  const original = Array.isArray(originalHeader) ? originalHeader[0] : originalHeader;
  ...
  if (
    currentPath === "/api" ||
    currentPath === "/api/index" ||
    currentPath.startsWith("/api/index.") ||
    currentPath === "/"
  ) {
